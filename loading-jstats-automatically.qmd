...
line that starts with `#` — holding the address of the install page, so the
reinstall instructions are one glance away on a new computer or after an R update.
Keep the `#` in place: without it, R would try to run the address as a command at
