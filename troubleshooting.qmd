...
- The blue box shows **no More info link at all**, even when you launch the installer
  from your Downloads folder. This one looks worse than it is: the block has been set
  centrally, so there's nothing to fix at your end — and it's a routine request for IT,
  often settled in a single phone call. Ask your IT service desk to unblock the R and
  RStudio installers, or to install them for you; going through them keeps everything in
  line with your organization's policies. The note below is written for whoever picks it
  up.
...
- **SmartScreen.** Microsoft SmartScreen may flag a freshly released version until it
  ...
  indication of malware. Where the user reports the warning with **no More info option
  at all**, that is typically SmartScreen configured to warn without a bypass rather
  than the default reputation check; the installers are code-signed and their provenance
  is above, so clearing it is at your discretion.
...
::: {.callout-warning collapse="true" title="R asked whether to install from sources, and I clicked Yes"}
The answer to that question is **No**. Nothing is broken if you clicked Yes — the
install may just stall, fail, or fill the Console with red text.

Run the install line on the [Install jstats](install-jstats.qmd) page again, and click
**No** this time.
:::
...
there can be more than one. That's normally invisible and harmless: jstats
installs into one of them and runs fine from there.
...
itself was updated**. Your installed packages live in a folder called a
**library**, and each version of R gets its own — so updating R starts a fresh,
empty one, and jstats (along with everything else you had installed) is no longer
there.
...
For the bigger picture on updating R — when to do it, and the fresh-library chore
that follows — see the [Updating](updating.qmd) page.
:::
